...
class SSD(nn.Module):
    ...
    def forward(self, x):
        ...
        for k in range(23, len(self.vgg)):
            x = self.vgg[k](x)
        s1_2 = x
        s1_1 = self.ffb[0](s1_1,s1_2)
        s1_1 = self.cfe[0](s1_1)
        sources.append(s1_1)
        x=self.cfe[1](x)
        ...
        for k, v in enumerate(self.extras):
            ...
            if k==0:
                s1_1=self.ffb[1](s1_2,x)
                s1_1=self.cfe[1](s1_1)
                sources.append(s1_1)
            if k==1:
                s1_2=x
                x=self.cfe[2](x)
                ...
                s1_2=self.ffb[nl](s1_2,x)
                s1_2=self.cfe[nl](s1_2)
                sources.append(s1_2)
...
cfe_channellist=[512,1024,512,256,256]
#change size!
# scalefact=0
# for i in base['300']:
#     if isinstance(i,'str'):
...
def build_ssd(phase, size=300, num_classes=21):
    ...
    cfes = []
    fbbs = []
    ffb_sizelist = [[512, 1024, (np.ceil(size/8).astype(np.int8), np.ceil(size/8).astype(np.int8))],
                    [1024, 256, (np.ceil(size/16).astype(np.int8),np.ceil(size/16).astype(np.int8))],
                    [512,256,(np.ceil(size/32).astype(np.int8),np.ceil(size/32).astype(np.int8))],
                    [256,256,(np.ceil(size/64).astype(np.int8),np.ceil(size/64).astype(np.int8))],
                    [256,256,(np.ceil(size/128).astype(np.int8),np.ceil(size/128).astype(np.int8))]]
