...
class CFE(nn.Module):
    def __init__(self,CFEl,CFEr,input_channel):
        ...
        self.conv_e=nn.Conv2d(input_channel, input_channel, kernel_size=1, padding=1)
        self.act_e=nn.ReLU(inplace=True)
    def forward(self, x):
        y=torch.cat((self.cfel(x),self.cfer(x)),1)
        y=self.conv_e(y)
        y=self.act_e(y)
        return x+y
...
def add_CFE_l(cfg, i):
    ...
    layers += [nn.Conv2d(cfg[1], cfg[2], kernel_size=(1, 7), stride=1, padding=(0,3))]
    layers += [nn.ReLU(inplace=True)]
    layers += [nn.Conv2d(cfg[2], cfg[3], kernel_size=(7, 1), stride=1, padding=(3,0))]
    layers += [nn.ReLU(inplace=True)]
...
def add_CFE_r(cfg, i):
    ...
    layers+=[nn.Conv2d(in_channels, cfg[1],kernel_size=(1,1))]
    layers += [nn.ReLU(inplace=True)]
    layers += [nn.Conv2d(cfg[1], cfg[2], kernel_size=(7, 1), stride=1, padding=(3,0))]
    layers += [nn.ReLU(inplace=True)]
    layers += [nn.Conv2d(cfg[2], cfg[3], kernel_size=(1, 7), stride=1, padding=(0,3))]
    layers += [nn.ReLU(inplace=True)]
    layers += [nn.Conv2d(cfg[3], cfg[4], kernel_size=(1,1)),nn.ReLU(inplace=True)]
    layers += [nn.ReLU(inplace=True)]
    return layers
...
cfe_config= {
    "300":[1024,512,512,512,512],
    "500":[],
}



cfe_channellist=[256,1024]
#change size!
# scalefact=0
# for i in base['300']:
#     if isinstance(i,'str'):
...
def build_ssd(phase, size=300, num_classes=21):
    ...
    cfes = []
    fbbs = []
    ffb_sizelist = [[256, 1024, (np.ceil(size/8), np.ceil(size/8))], [1024, 1024, (np.ceil(size/16),np.ceil(size/16))]]
    for i in cfe_channellist:
        cfes.append(CFE(add_CFE_l(cfe_config[str(size)],i),add_CFE_r(cfe_config[str(size)],i),i))
